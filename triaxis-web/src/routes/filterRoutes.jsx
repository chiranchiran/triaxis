...
export const filterRoutes = (routes, currentRole, parentAllowedRoles = []) => {
  return routes.map(route => {
    const allowedRoles = route.allowedRoles || parentAllowedRoles;
    // 处理子路由（递归）
    const children = route.children
      ? { children: filterRoutes(route.children, currentRole, allowedRoles) }
      : {};
    console.log(`路由: ${route.path || '根路由'}, allowedRoles:`, allowedRoles);
    // 公开路由直接返回
